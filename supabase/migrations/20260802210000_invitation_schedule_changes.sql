-- نظام إلغاء / إعادة جدولة الدعوات الخاصة:
-- حقول الحالة على الدعوة + سجل كامل بالتعديلات + تحديث دالة الرابط العام
-- ليشاهد المدعوون إشعار الإلغاء أو الموعد الجديد فور فتح دعوتهم.
...
CREATE TABLE IF NOT EXISTS public.invitation_schedule_changes (
  ...
  new_event_date timestamptz,
  created_by uuid NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.invitation_schedule_changes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Org owner views invitation schedule log" ON public.invitation_schedule_changes;
CREATE POLICY "Org owner views invitation schedule log" ON public.invitation_schedule_changes
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.private_invitations i
      JOIN public.organizations o ON o.id = i.organization_id
      WHERE i.id = invitation_schedule_changes.invitation_id AND o.owner_id = auth.uid()
    )
    OR EXISTS (
...
DROP POLICY IF EXISTS "Org owner logs invitation schedule change" ON public.invitation_schedule_changes;
CREATE POLICY "Org owner logs invitation schedule change" ON public.invitation_schedule_changes
...
-- تحديث دالة جلب الدعوة بالرمز لتشمل حقول الإلغاء / التأجيل
